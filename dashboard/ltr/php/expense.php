<?php
	...
	if($ref==1)
	{
		...
		$data['expense_details'] = null;
		$data['total'] = null;
		
		$sql = $conn->prepare("SELECT * FROM expense WHERE bId = ?");
	...
	else if($ref==2)
	{
		...
		if($update==0)
		{
			$sql = $conn->prepare("INSERT INTO expense(note, amt, date, bId) 
			VALUES (?,?,?,?)");
		...
		else
		{
			...
			$sql = $conn->prepare("UPDATE expense SET 
	...
	else if($ref==3)
	{
		...
		$sql = $conn->prepare("SELECT * FROM expense WHERE bId = ? AND id = ?");
	...
	else if($ref==4)
	{
		...
		$sql = $conn->prepare("DELETE FROM expense WHERE id = ?");
		$sql->bind_param("i", $id);
