<?php
	...
	if ($conn->connect_error) {
		...
		$sql1 = $conn->prepare("SELECT send_to.messageId, send_to.memberId, members.roomNo, message.message, message.date, message.time
						FROM send_to INNER JOIN message
						ON send_to.messageId = message.id INNER JOIN members 
						ON members.id = send_to.memberId
						WHERE message.bId = ? AND send_to.memberId = ?");
		$sql1->bind_param("ii",$b_selected, $memberId);
